...
export const saveMealToDB = (userId, meal) => {
  ...
  return db.runSync(
    ...
    [
      ...
      meal.grade || 'B', 
      meal.recommendation || 'Logged entry', 
      entryDate
    ]
  );
};
...
export const getAllMealDates = (userId) => {
  const results = db.getAllSync(
    'SELECT DISTINCT date FROM nutrition_logs WHERE userId = ? ORDER BY date DESC',
    [userId]
  );
...
export const getWorkoutStreak = (userId) => {
  const workouts = db.getAllSync(
    "SELECT DISTINCT SUBSTR(timestamp, 1, 10) as workoutDate FROM workouts WHERE userId = ? AND status = 'Complete' ORDER BY workoutDate DESC",
    [userId]
  );
...
export const getTodayTotalCalories = (userId) => {
  ...
  const result = db.getFirstSync(
    'SELECT SUM(calories) as total FROM nutrition_logs WHERE userId = ? AND date = ?',
    [userId, today]
  );
...
export const getWeeklyWorkoutDistribution = (userId) => {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const row = db.getFirstSync(
      "SELECT COUNT(*) as count FROM workouts WHERE userId = ? AND timestamp LIKE ? AND status = 'Complete'",
      [userId, `${dateStr}%`]
    );
...
export const getTotalQuestCount = (userId) => {
  const result = db.getFirstSync(
    'SELECT COUNT(*) as total FROM daily_quests WHERE userId = ?',
    [userId]
  );
  return result?.total || 0;
};
...
export const getDetailedQuestHistory = (userId) => {
  return db.getAllSync(
    'SELECT questId, completionDate FROM daily_quests WHERE userId = ? ORDER BY completionDate DESC',
    [userId]
  );
};
...
export const getLifetimeNutrients = (userId) => {
  return db.getFirstSync(
    'SELECT SUM(calories) as cal, SUM(protein) as pro, SUM(carbs) as carb, SUM(fat) as fat FROM nutrition_logs WHERE userId = ?',
    [userId]
  );
};
...
export const updateUserProfile = (userId, username, age, weight, height, goal) => {
  return db.runSync(
    'UPDATE users SET username = ?, age = ?, weight = ?, height = ?, fitnessGoal = ? WHERE id = ?',
    [username, age, weight, height, goal, userId]
  );
};
